...
export default {
  ...
  async fetch() {
    try {
      ...
      if (customerId) {
        const messageResponse = await this.getCustomerMessage(customerId)
      ...
      // Remove from parameter if no existing customer from customer is found
      if (!this.customerId) {
  ...
  methods: {
    ...
    async sendNewMessage(body) {
      // Send new message via webhook API
